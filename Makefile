...
RM     = rm -f
MD     = mkdir -p
CC     = /opt/gkd350-toolchain/usr/bin/mipsel-linux-gcc
CXX    = /opt/gkd350-toolchain/usr/bin/mipsel-linux-g++
LD     = /opt/gkd350-toolchain/usr/bin/mipsel-linux-g++

SYSROOT     := /opt/gkd350-toolchain/usr/mipsel-gkd350-linux-uclibc/sysroot/usr/bin
SDL_CONFIG  := $(SYSROOT)/sdl-config
SDL_CFLAGS  := $(shell $(SDL_CONFIG) --cflags)
SDL_LIBS    := $(shell $(SDL_CONFIG) --libs)

LDFLAGS := $(SDL_LIBS) -lrt -lz -lintl
...
ifeq ($(DEV),1)
	...
	LDFLAGS = $(SDL_LIBS) -lrt -lz -lintl -Wl,--as-needed -Wl,--gc-sections -s
	LDFLAGS += -flto
	# Furthermore, div-by-zero checks are disabled (helps software rendering)
	C_ARCH  += -mno-check-zero-division
endif

CFLAGS := $(C_ARCH) -mplt -mno-shared -ggdb3 -O2 -flto -DTIME_IN_MSEC \
